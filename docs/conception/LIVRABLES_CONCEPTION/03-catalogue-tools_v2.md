...
> [`03-catalogue-tools_v2.drawio`](03-catalogue-tools_v2.drawio) ; ce fichier porte ce qu'un
...
> ---
>
> **v2 — 2026-09-13.** Six énoncés de la v1 ont été alignés sur le code **effectivement
> servi** : trois noms d'arguments, la sortie de `list_sources`, le refus d'`ask_database`
> et le nombre de contrôles. Le détail, et ce qui reste divergent sans être corrigé ici,
> sont en [§9](#9-écarts-relevés-en-écrivant-ce-catalogue). La v1 est conservée telle quelle
> dans [`03-catalogue-tools.md`](03-catalogue-tools.md).
...
| **Identité** | il sait ce qu'il veut et le nomme — par `doc_id` + `version`, **jamais** par référence produit | `get_document` · `list_sources` | « la notice de REF-8842, version 1.0 » |
...
| `search_docs` ↔ `get_document` | similarité vs adressage explicite par `doc_id` + `version` | l'édition antérieure reste inatteignable — 50 documents ont deux éditions |
...
| `introuvable` | | non | `doc_id` hors corpus, ou version inexistante — `get_document` |
...
| | |
|---|---|
| **Entrée** | `query` (texte libre) · `collections` (optionnel) |
| **Sortie** | une liste d'extraits, chacun avec ses **onze métadonnées** et son rang |
| **Périmètre** | identique à `answer_question` — même `perimeter_for(profil)` |
...
## `get_document(doc_id, version)`

**À quoi il sert.** Rendre une édition précise, nommée.

| | |
|---|---|
| **Entrée** | `doc_id` (obligatoire — un `edition_id` comme `fiches/REF-8842-v2.1`, ou un `doc_key` comme `fiches/REF-8842`) · `version` (optionnel — l'édition courante par défaut) |
| **Sortie** | le texte de l'édition, **liens sortants inclus**, avec ses métadonnées |
| **Périmètre** | décidé **sur le seul argument, avant tout accès à l'index** : `doc_id` porte la collection et le thème, tous deux dérivés du nom de fichier par `classify()` |
...
| **Sortie** | `sources` : la **liste plate** des éditions courantes autorisées, triée par `doc_id`. Par édition : `doc_id`, `doc_type`, `titre`, `reference`, `version`, `date`, `doc_key`. **Ni regroupement par collection, ni décompte, ni plage de dates** — un client qui veut ces trois-là les calcule depuis la liste |
| **Périmètre** | construit **depuis `perimeter_for(profil)`**, jamais par un parcours du corpus |
...
| le refus est lisible, motivé et journalisé, et **il ne nomme pas la colonne** au client | phrase figée (« Cette information n'est pas accessible avec votre profil ») ; le nom de la colonne part au **journal** seulement. L'énumérer ferait du refus un oracle : en sondant, on cartographierait la matrice |
| la requête est confrontée au moteur **avant** d'être exécutée | **contrôle 6** — `EXPLAIN` à blanc, après la borne. Il prépare la requête sans lire une ligne, et dit « ça se prépare », jamais « c'est permis ». Sur échec, **une** reprise — jamais sur un refus de droits |
...
| **Entrée** | `reference` — `"pattern": "^REF-\\d{4}$"`, validé **avant** exécution ; un libellé est refusé |
...
### Écarts au code servi — relevés le 2026-09-13, corrigés dans cette v2

Le code a été confronté cellule par cellule à ce catalogue. Six énoncés étaient faux.

| # | Énoncé de la v1 | Ce que le code sert | Source |
|---|---|---|---|
| 1 | `search_docs(question, …)` | `search_docs(query, …)` | `mcp_server/server.py:524` |
| 2 | `get_document(doc_key, …)` | `get_document(doc_id, …)` — accepte un `edition_id` **ou** un `doc_key` | `server.py:533` · `rag_machines/tools.py:290` |
| 3 | `check_stock(ref)` | `check_stock(reference)` | `server.py:575` |
| 4 | `list_sources` rend un inventaire **par collection**, avec décomptes et plage de dates | une **liste plate** d'éditions courantes | `rag_machines/tools.py:427` · `mcp_server/output_schemas.py` |
| 5 | le refus d'`ask_database` **nomme la colonne** | phrase figée ; le nom part au journal via l'attribut `forbidden`, qui ne traverse jamais `client_view()` | `text_to_sql_factory/structured_answer.py:97` |
| 6 | « cinq contrôles » | cinq contrôles d'arbre, **puis la borne, puis le contrôle 6 `EXPLAIN`** | `text_to_sql_factory/validator.py:18` |

Les points 1 à 3 relèvent de la même règle d'arbitrage du dépôt : **le test fait foi**, et
les noms d'arguments suivent la suite d'acceptance. Les points 1 et 2 étaient déjà consignés
au journal de développement ; **le point 3 ne l'était pas.**

Le point 5 n'est pas une coquille : la v1 promettait exactement ce que le code **refuse**
de faire, et pour un motif de sécurité écrit dans le code.

`filtre_index(profil)` a été remplacé par `perimeter_for(profil)` : le premier nom ne vit
que dans `Q3.md`, le second est celui du code. La propriété décrite, elle, était juste.

### Ce qui reste divergent, et n'est pas corrigé ici

Relevé en même temps, laissé tel quel — ce sont des arbitrages déjà tranchés et consignés,
pas des erreurs de rédaction :

- **les noms de champs de l'enveloppe** (§4) : le conçu dit `code` / `hint` / `reponse` /
  `citations` à la racine, le servi dit `status` / `payload` / `message`, avec `code` **dans**
  le payload. `hint` n'est jamais servi, et son rôle est tenu par les descriptions de tools ;
- **`isError` tranché code par code** (§5) : le serveur ne le pose que sur
  `erreur_execution`. Un refus de droits n'a rien à corriger, et le marquer inviterait le
  client à réessayer à l'identique — la différence entre un `500` et un `403` ;
- **la citation** (§4) : le servi exige **cinq** clés dont `reference`, garantie non vide
  (elle retombe sur `doc_key`), là où la v1 en exige quatre et dit `reference` omise sur 210
  éditions. Et `additionalProperties` n'est fermé nulle part dans le servi ;
- **les listes de codes des fiches** : `list_sources` sert aussi `aucune_ligne`,
  `perimetre_interdit` et `erreur_execution`.

Ces quatre points demandent une décision, pas une correction de texte.

Un point ouvert, non tranché ici : le `top_k` de `search_docs` (voir sa fiche).

## Renvois

- tableau du catalogue : [`03-catalogue-tools_v2.drawio`](03-catalogue-tools_v2.drawio)
