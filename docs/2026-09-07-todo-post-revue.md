...
| 4b | ~~**2bis.1** saturation par redondance~~ | un profil à **plus** de droits obtient **moins** de réponses : c'est E1 qui recule là où la matrice s'élargit | **faite le 2026-09-09** — vivier 60 / budget 20 / plafond 3, les quatre profils coïncident à 3/4 ; axe 8 publié. **La propriété reste à contrôler** : les 12 contrôles portent sur le mécanisme, pas sur la monotonie |
| 5b | **2bis.2** le seuil coupe dans le couvert | refuse une question dont le corpus porte la réponse, **sans que le juge la voie** | **ouvert**, et **isolé par 4b** : sur `CND-02`, `support` rejoint exactement `dev` (0,0021 → 0,0061) — le périmètre est réparé, c'est le seuil qui coupe. Trois tournures du même document : 0,9480 · 0,0859 · 0,0061, **facteur 155**. À trancher avec §2.4 |
